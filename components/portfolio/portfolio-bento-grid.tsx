    ...
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 md:gap-6 auto-rows-auto">
      {items.map((item, index) => {
        ...
        return (
          <div
            key={item.slug}
            className={`${gridSpan} group relative overflow-hidden rounded-3xl border border-border bg-card transition-all duration-300 hover:shadow-md flex flex-col`}
          >
              ...
              <div className="flex flex-col h-full">
                <div className="flex items-center gap-2 mb-2">
                  {item.categories && item.categories.length > 1 && (
                    <span className="px-2 py-1 bg-muted rounded-full text-xs text-foreground whitespace-nowrap">
                      {item.categories[1].charAt(0).toUpperCase() + item.categories[1].slice(1)}
                    </span>
                  )}
                </div>
                <h3
                  className={`font-medium text-foreground ${isLarge ? "text-xl md:text-2xl" : "text-lg"} mb-2`}
                >
                  {item.title}
                </h3>
